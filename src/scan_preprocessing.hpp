...
namespace sick_ros
{
    struct EIGEN_ALIGN16 MS136Point
    {
        PCL_ADD_POINT4D;
        float i;
        float range;
...
POINT_CLOUD_REGISTER_POINT_STRUCT(sick_ros::MS136Point,
    (float, x, x)
    (float, y, y)
    (float, z, z)
    (float, i, i)
...
public:
    ...
    int lidar_type, point_filter_num, time_unit, N_SCANS, SCAN_RATE;
    double blind;
...
private:
    ...
    void ms136_handler(const sensor_msgs::msg::PointCloud2::UniquePtr& msg);
    ...
    void pub_func(PointCloudType &pl, const rclcpp::Time &ct);
