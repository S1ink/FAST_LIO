...
// void Preprocess::process(const livox_ros_driver2::msg::CustomMsg::UniquePtr &msg, PointCloudType::Ptr& pcl_out)
// {
//   avia_handler(msg);
//   *pcl_out = pl_surf;
// }

void Preprocess::process(const sensor_msgs::msg::PointCloud2::UniquePtr &msg, PointCloudType::Ptr& pcl_out)
{
    switch (time_unit)
    {
        case SEC:
...
void Preprocess::ms136_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
    pl_surf.clear();
    pl_corn.clear();
    pl_full.clear();
    pcl::PointCloud<sick_ros::MS136Point> pl_orig;
    pcl::fromROSMsg(*msg, pl_orig);
    ...
    if(feature_enabled)
    {
        ...
        for(size_t i = 0; i < plsize; i++)
        {
            ...
            if(orig_pt.range < blind) continue;

            static uint8_t LAYER_INDICES[] =
                { 0, 1, 1, 2, 2, 3, 3, 4, 5, 6, 6, 7, 7, 8, 8, 9, 9, 10, 10, 11, 11, 12, 13, 14, 14, 15 };
            const uint8_t layer =
                LAYER_INDICES[ static_cast<int>( std::round(    // https://www.desmos.com/calculator/jijsicsofw
                    (static_cast<double>(orig_pt.elevation) + 0.387463093943) * 23.2437239404 ) ) ];

            pl_buff[layer].emplace_back();
            PointType& added_pt = pl_buff[layer].back();
        ...
        {
            PointCloudType& pl = pl_buff[j];
            size_t linesize = pl.size();
            if(!linesize) continue;
    ...
    else
    {
        ...
        for(size_t i = 0; i < plsize; i++)
        {
            ...
            pl_surf.emplace_back();
            PointType& added_pt = pl_surf.back();
...
void Preprocess::pub_func(PointCloudType& pl, const rclcpp::Time& ct)
{
    pl.height = 1;
    pl.width = pl.size();
